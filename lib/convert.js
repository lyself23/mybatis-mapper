...
var replaceEvalString = function(evalString, param) {
  ...
  if(evalString.includes('param.') && evalString.includes('!= null')) {
    var dynamicPattern = new RegExp('([^!=]+)(?=\\s*!=)');
    var match = evalString.match(dynamicPattern);
    var value = eval(match[1].trim())
  
    if(value === undefined) {
      evalString = evalString.replace('param.', '')
    }
  }
  return evalString;
}
